...
const dataToMarkdown = (dataObj) => {
  ...
  if (
    !dataObj.hasOwnProperty('content') ||
    !dataObj.content ||
    dataObj.content.length < 1
  ) {
    dataObj.content = ' ';
  }
...
const processObjectToMarkdown = (
  ...
  if (fs.existsSync(`${pathString}/${fileSlug}.md`)) {
    ...
    let oldMarkdown = '';
    try {
      oldMarkdown = fs.readFileSync(`${pathString}/${fileSlug}.md`, 'utf8');
    } catch (e) {
    ...
    if (oldData) {
      ...
      if (
        ((oldData.content.length > 1 && content?.length <= 3) ||
          oldData.content == content) &&
        deepEqual(newData, oldData.data)
      ) {
        return true;
      }
      ...
      return writeDataToMarkdown(pathString, fileSlug, obj, content, false);
    }
  }
